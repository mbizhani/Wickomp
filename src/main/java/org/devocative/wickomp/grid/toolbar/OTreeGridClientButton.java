	...
	public String getHTMLContent(WGridInfo<T> gridInfo) {
		...
		if (expand != null) {
			builder
				...
				.append("\" onclick=\"")
				.append(String.format("$('#%s').treegrid('expandAll');", gridInfo.getOptions().getGridHTMLId()))
				.append("\">")
		...
		builder
			...
			.append("\" onclick=\"")
			.append(String.format("$('#%s').treegrid('collapseAll');", gridInfo.getOptions().getGridHTMLId()))
			.append("\">")
