...
public class WickompApplication extends WebApplication {
	...
	public void init() {
		...
		AsyncMediator.init(this, false, false);
		...
		AsyncMediator.registerHandler("T_GRID_PAGER", (token, requestPayLoad) -> {
		...
		AsyncMediator.registerHandler("T_GRID_CHILDREN", (asyncToken, requestPayLoad) -> {
			...
			new Thread() {
				...
				public void run() {
					...
					AsyncMediator.sendResponse(asyncToken, ObjectBuilder
						.<String, Object>createDefaultMap()
		...
		AsyncMediator.registerHandler("GRID_PAGER", (token, requestPayLoad) -> {
			Map<String, Object> map = (Map<String, Object>) requestPayLoad;
			final long first = (long) map.get("first");
			long size = (long) map.get("size");
